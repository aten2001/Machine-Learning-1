...
/*
 * 
 *  Adapted from https://github.com/kylewest520/CS-7641---Machine-Learning by Kyle West (in turn adapted from AbaloneTest.java by Hannah Lau)
 */
...
public class phishing_sa_val_iter {
    ...
    private static int inputLayer = 30, hiddenLayer=50, outputLayer = 1;
    ...
    public static void main(String[] args) {
        ...
        int[] iterations = {10,100,500,2500,5000,10000,20000,50000};
		//int[] iterations = {50000};
		//Optimum Values = 0.8, 1.00E+12

		//int [] iterations = {1000};
		
        //double[] coolings = {.35,.65,.80,.95};
		//double[] temperatures = {1e10,1e11,1e12};
		double[] coolings = {0.80};
		double[] temperatures = {1e12};
        ...
        for (int trainingIterations : iterations) {
			...
			for(int k = 0; k < temperatures.length; k++) {
				...
				//for (double cooling : coolings) {
					...
					oa[0] = new SimulatedAnnealing(temperatures[k], coolings[q], nnop[0]);
					...
					results += "\nTrain Results for SA:" + "," + coolings[q] + "," + temperatures[k] + "," + ": \nCorrectly classified " + correct + " instances." +
                        "\nIncorrectly classified " + incorrect + " instances.\nPercent correctly classified: "
                        + df.format(correct / (correct + incorrect) * 100) + "%\nTraining time: " + df.format(trainingTime)
                        + " seconds\nTesting time: " + df.format(testingTime) + " seconds\n";

					final_result = oaNames[0] + "," + trainingIterations + "," + coolings[q] + "," + temperatures[k] + "," + "training accuracy" + "," + df.format(correct / (correct + incorrect) * 100)
                        + "," + "training time" + "," + df.format(trainingTime) + "," + "testing time" +
                        "," + df.format(testingTime);
					write_output_to_file("Optimization_Results", "phishing_results_sa_iter.csv", final_result, true);
					...
					results += "\nTest Results for SA: " + coolings[q] + "," + temperatures[k] + ": \nCorrectly classified " + correct + " instances." +
                        "\nIncorrectly classified " + incorrect + " instances.\nPercent correctly classified: "
                        + df.format(correct / (correct + incorrect) * 100) + "%\nTraining time: " + df.format(trainingTime)
                        + " seconds\nTesting time: " + df.format(testingTime) + " seconds\n";

					final_result = oaNames[0] + "," + trainingIterations + "," + coolings[q] + "," + temperatures[k] + "," + "testing accuracy" + "," + df.format(correct / (correct + incorrect) * 100)
                        + "," + "training time" + "," + df.format(trainingTime) + "," + "testing time" +
                        "," + df.format(testingTime);
					write_output_to_file("Optimization_Results", "phishing_results_sa_iter.csv", final_result, true);
				}
    ...
    private static Instance[] initializeInstances() {
        ...
        try {
            BufferedReader br = new BufferedReader(new FileReader(new File("C:/Users/kunal/Desktop/FALL18/ML/HW2/Phising_dataset.csv")));
			//br.readLine();
            //for each sample
